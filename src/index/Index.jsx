...
import useAuthStore from "../stoer/AuthStore";
...
function Login() {
  ...
  const loginCheck = () => {
    axios
      .post("http://10.10.55.80/auth/login", idAndPw, { withCredentials: true })
      .then((resp) => {
  ...
  //테스트 버튼
  const test = () => {
    axios.get("http://10.10.55.80//auth/test", { withCredentials: true });
  };

  return (
    <>
      {islogin ? (
        <div>로그인 완료! 홈으로 이동합니다.</div>
      ) : (
