...
function Home (){
    ...
    //회원탈퇴 버튼 클릭
    const signout=()=>{
        axios.delete(`http://10.5.5.3/auth/${user}`).then(
            ()=>{
                deleteUser(); 
            }
        )   
        ...
        <div className={styles.homebox}>
            <table border={1}>
                ...
                <tbody>
                    ...
                    <tr>
                    <td> <button onClick={() => navigate("/board")}>게시판</button> </td>
                    <td> <button onClick={() => navigate("/mypage")}>마이페이지</button> </td>
                    <td> <button onClick={logout}>로그아웃</button> </td>
                    <td> <button onClick={signout}>회원탈퇴</button> </td>
                    </tr>
